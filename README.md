...
**[Download SPIN.app beta for Mac](https://github.com/claudiaclawdbot/spin/releases/tag/v4.1.0-beta.3)** ·
...
1. Download the beta from [v4.1.0-beta.3](https://github.com/claudiaclawdbot/spin/releases/tag/v4.1.0-beta.3).
...
When Codex Computer Use is installed, SPIN delegates bounded desktop tasks from
OMP to the signed Codex CLI. Codex then owns its supported `node_repl` plugin
wrapper and native service trust chain. OMP 16.4 incorrectly imports Codex's
disabled direct `computer-use` MCP, so SPIN suppresses that server in
`~/.omp/agent/mcp.json`; OMP's own `node_repl` is not advertised as a desktop
bridge. The repair is idempotent and preserves unrelated or custom MCP servers.
`spin doctor` reports configuration truth, while `spin computer-use probe`
performs the read-only live proof required before calling the lane ready.
