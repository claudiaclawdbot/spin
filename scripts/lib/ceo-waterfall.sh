...
#   mark_codex_blocked [secs]   -> write an owner-controlled lockout (default 24h)
...
SPIN_OMP_CONFIG="${SPIN_OMP_CONFIG:-}"
...
# Returns an owner-selected config path verbatim, or a lane-isolated generated
# path. A lane is trusted control-plane context, never project.env input.
spin_omp_config_path() {
  local lane="${1:-${SPIN_OMP_CONFIG_LANE:-workspace-agent}}"
  if [[ -n "${SPIN_OMP_CONFIG:-}" ]]; then
    printf '%s\n' "$SPIN_OMP_CONFIG"
    return 0
  fi
  if [[ ${#lane} -gt 200 || "$lane" == "." || "$lane" == ".." || ! "$lane" =~ ^[A-Za-z0-9._:-]+$ ]]; then
    echo "invalid OMP config lane: $lane" >&2
    return 2
  fi
  printf '%s/omp-configs/%s.yml\n' "$CEO_RUN_DIR" "$lane"
}

# Writes a lane-specific SPIN OMP config overlay and echoes its path. OMP still
# owns credentials, per-account usage backoff, and model fallback at runtime;
# SPIN only declares the roles/chains it wants for coordinator/project work.
ensure_spin_omp_config() {
  local lane="${1:-${SPIN_OMP_CONFIG_LANE:-workspace-agent}}"
  local omp_config
  omp_config="$(spin_omp_config_path "$lane")" || return $?
  ...
  if ! mkdir -p "$(dirname "$omp_config")"; then
    echo "cannot create OMP config directory: $(dirname "$omp_config")" >&2
    return 1
  fi
  ...
  local tmp
  tmp="$(mktemp "${omp_config}.tmp.XXXXXX")" || {
    echo "cannot create temporary OMP config for: $omp_config" >&2
    return 1
  }
  if ! {
...
# --- generic provider lockout (usage-limit preflight) ----------------------
# Owner- or control-plane-written lockouts keep a known-limited provider out of
# future runs. Runtime task logs are deliberately not used to create lockouts:
# provider output is untrusted task content and a failed attempt may already have
# changed project files. Codex keeps its original lockout path for back-compat.
_provider_lock_file() {
  case "$1" in
...
# Returns the CLI's exit code. Task output never changes retry classification.
run_agent() {
  local provider="$1"; local prompt="$2"; local log="$3"; shift 3
  local rc=0 agent_cwd
  agent_cwd="${SPIN_AGENT_CWD:-$CEO_ROOT}"
  [[ -d "$agent_cwd" ]] || {
    echo "run_agent: working directory does not exist: $agent_cwd" >&2
    return 1
  }
  agent_cwd="$(cd -P "$agent_cwd" && pwd)"

  case "$provider" in
    codex)
      local codex_bin="" codex_cmd=()
      if codex_bin="$(spin_resolve_codex_cli 2>/dev/null)"; then
        codex_cmd=("$codex_bin" exec --cd "$agent_cwd" --sandbox workspace-write)
        ...
        codex_cmd=(omx exec --cd "$agent_cwd" --sandbox workspace-write)
        [[ -n "${CEO_CODEX_MODEL:-}" ]] && codex_cmd+=(--model "$CEO_CODEX_MODEL")
        for d in "$@"; do codex_cmd+=(--add-dir "$d"); done
        codex_cmd+=(-)
        echo "$prompt" | "${codex_cmd[@]}" > "$log" 2>&1 || rc=$?
      ...
      ;;
    claude)
      local add_args=(--add-dir "$agent_cwd")
      for d in "$@"; do add_args+=(--add-dir "$d"); done
      ( cd "$agent_cwd" && claude -p "$prompt" --model "${MODEL:-$CEO_CLAUDE_MODEL}" \
          --permission-mode dontAsk "${add_args[@]}" ) \
          > "$log" 2>&1 || rc=$?
      ;;
    cursor)
      echo "$prompt" | cursor-agent -p --trust --sandbox enabled \
        --workspace "$agent_cwd" --model "${MODEL:-$CEO_CURSOR_MODEL}" \
        > "$log" 2>&1 || rc=$?
      ;;
    gemini)
      ( cd "$agent_cwd" && echo "$prompt" | gemini --model "${MODEL:-$CEO_GEMINI_MODEL}" ) \
        > "$log" 2>&1 || rc=$?
      ;;
    omp)
      local omp_config computer_use_prompt
      local omp_args=()
      omp_config="$(ensure_spin_omp_config "${SPIN_OMP_CONFIG_LANE:-workspace-agent}")" || return $?
      computer_use_prompt="$(spin_omp_computer_use_prompt)"
      omp_args=(-p --config "$omp_config" --cwd "$agent_cwd" --no-session)
      [[ -n "$computer_use_prompt" ]] && omp_args+=(--append-system-prompt "$computer_use_prompt")
      spin_cmd omp "${omp_args[@]}" "$prompt" > "$log" 2>&1 || rc=$?
      ;;
    ollama)
      ( cd "$agent_cwd" && echo "$prompt" | ollama run "${MODEL:-$CEO_OLLAMA_MODEL}" ) \
        > "$log" 2>&1 || rc=$?
      ;;
    *)
...
# Bound each outer provider attempt so a hung harness cannot consume the whole
# job budget and prevent the documented compatibility fallback from running.
agent_provider_timeout_seconds() {
  local configured="${CEO_PROVIDER_TIMEOUT_SECS:-${OMP_JOB_PROVIDER_TIMEOUT_SECS:-}}"
  if [[ "$configured" =~ ^[1-9][0-9]*$ ]]; then
    printf '%s\n' "$configured"
    return
  fi

  local budget="${OMP_JOB_MAX_RUNTIME_SECONDS:-}"
  if [[ "$budget" =~ ^[1-9][0-9]*$ ]]; then
    local deadline=$(( (budget - 120) / 3 ))
    (( deadline < 60 )) && deadline=60
    (( deadline > 1800 )) && deadline=1800
    printf '%s\n' "$deadline"
    return
  fi
...
# A direct-provider retry can duplicate partial project work. Only advance the
# outer chain when the harness did not get a usable run: timeout or missing or
# non-executable CLI. Provider output is untrusted task output,
# so log text must never classify a failure as safe to retry. Ordinary task
# failures stay with their original provider and surface to the supervisor.
provider_failure_allows_fallback() {
  local provider="$1" rc="$2"
  (( rc == 124 || rc == 126 || rc == 127 )) && return 0
  # OMP reserves 2 for fail-fast CLI usage errors detected before a session or
  # initial prompt can run. It is therefore a harness failure, not partial work.
  [[ "$provider" == "omp" && "$rc" == "2" ]] && return 0
  return 1
}
...
run_agent_resilient() {
  ...
  local tried=" " provider rc attempt_log
  : > "$log"
  for provider in "${candidates[@]}"; do
    ...
    provider_timeout="$(agent_provider_timeout_seconds)"
    echo "  [waterfall] trying $provider (timeout ${provider_timeout}s)" >&2
    attempt_log="${log}.${provider}.attempt"
    rm -f "$attempt_log"
    rc=0
    run_with_timeout "$provider_timeout" run_agent "$provider" "$prompt" "$attempt_log" "$@" || rc=$?
    {
      printf '\n===== provider: %s (rc=%s) =====\n' "$provider" "$rc"
      [[ -f "$attempt_log" ]] && cat "$attempt_log"
    } >> "$log"

    if (( rc == 0 )); then
      return 0
    fi
    if provider_failure_allows_fallback "$provider" "$rc" "$attempt_log"; then
      if (( rc == 124 )); then
        echo "  [waterfall] $provider timed out after ${provider_timeout}s — next provider" >&2
      else
        echo "  [waterfall] $provider hard-failed (rc=$rc) — next provider" >&2
      fi
      continue
    fi
    echo "  [waterfall] $provider task failed (rc=$rc); refusing a duplicate outer run" >&2
    return "$rc"
  done
...
# freeze the driver loop. The subshell keeps job-control and signal traps local;
# monitor mode gives the command and watcher independent process groups so a
# child that ignores TERM cannot escape when its shell wrapper exits first.
run_with_timeout() (
  local secs="$1"; shift
  local marker='' cmd_pid='' cmd_pgid='' watcher_pid='' watcher_pgid='' rc=0

  marker="$(mktemp "${TMPDIR:-/tmp}/spin-provider-timeout.XXXXXX")" || return 1

  timeout_group_alive() {
    [[ -n "$1" ]] && kill -0 -- "-$1" 2>/dev/null
  }

  timeout_wait_for_group() {
    local pgid="$1" pid="$2" checks=100
    while (( checks > 0 )); do
      timeout_group_alive "$pgid" && return 0
      kill -0 "$pid" 2>/dev/null || return 1
      sleep 0.01
      checks=$((checks - 1))
    done
    timeout_group_alive "$pgid"
  }

  timeout_cleanup() {
    trap - EXIT HUP INT TERM
    if [[ -n "$watcher_pgid" ]]; then
      kill -s TERM -- "-$watcher_pgid" 2>/dev/null || true
      kill -s KILL -- "-$watcher_pgid" 2>/dev/null || true
    fi
    if [[ -n "$cmd_pgid" ]]; then
      kill -s TERM -- "-$cmd_pgid" 2>/dev/null || true
      kill -s KILL -- "-$cmd_pgid" 2>/dev/null || true
    fi
    rm -f "$marker"
  }
  ...
  # Bash monitor mode assigns each background job a process group whose leader
  # is the PID exposed through $!. Launch both jobs before restoring the default.
  set -m
  (
    # Keep any background work started by the command inside this group too.
    set +m
    "$@"
  ) &
  cmd_pid=$!
  ...
  # If job-control isolation unexpectedly failed, stop the wrapper by PID and
  # fail closed instead of risking a negative signal against the caller's group.
  if ! timeout_wait_for_group "$cmd_pgid" "$cmd_pid" && kill -0 "$cmd_pid" 2>/dev/null; then
    set +m
    echo "run_with_timeout: could not isolate command process group" >&2
  ...
  (
    set +m
    sleep "$secs"
    if timeout_group_alive "$cmd_pgid"; then
      printf 'timed-out\n' > "$marker"
      kill -s TERM -- "-$cmd_pgid" 2>/dev/null || true

      # Give cooperative processes up to five seconds to exit. Checking the
      # group, rather than only its leader, keeps reparented descendants visible.
      grace_checks=50
      while (( grace_checks > 0 )) && timeout_group_alive "$cmd_pgid"; do
        sleep 0.1
  ...
  watcher_pid=$!
  watcher_pgid=$watcher_pid
  if ! timeout_wait_for_group "$watcher_pgid" "$watcher_pid"; then
    set +m
    echo "run_with_timeout: could not isolate timeout watcher process group" >&2
    kill -s TERM "$watcher_pid" 2>/dev/null || true
    kill -s KILL "$watcher_pid" 2>/dev/null || true
  ...
  if [[ -s "$marker" ]]; then
    # The leader can exit on TERM while an ignoring descendant survives. Let the
    # watcher finish the grace period and group-wide KILL before returning 124.
    wait "$watcher_pid" 2>/dev/null || true
    rc=124
  else
    # Normal completion: cancel the whole watcher group, including its sleep.
    kill -s TERM -- "-$watcher_pgid" 2>/dev/null || true
    wait "$watcher_pid" 2>/dev/null || true
  fi
